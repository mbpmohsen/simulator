...
export default function AiAssistantLevels({
	expectedGameId = null,
	embedded = false,
}: AiAssistantLevelsProps) {
	...
	return (
		<div className="space-y-5">
			...
			{gameMismatch && (
				<div className="flex flex-wrap items-start gap-3 rounded-2xl border border-rose-400/30 bg-rose-500/10 p-4 text-sm leading-7 text-rose-100">
					<AlertTriangle className="mt-1 size-4 shrink-0" />
					<span>
						این تنظیمات به بازی{" "}
						...
						تعلق دارد، اما آخرین بازی منتشرشدهٔ شما{" "}
						<span dir="ltr" className="font-mono">
							{expectedGameId}
						</span>{" "}
						است. ذخیره در این حالت تنظیمات بازی دیگری را تغییر می‌دهد.
					</span>
				</div>
			)}
				...
				<CardContent className="space-y-4 p-5">
					...
					<div className="flex flex-col gap-3 border-t border-white/10 pt-4 lg:flex-row lg:items-center lg:justify-between">
						<div className="text-sm text-slate-400">
							{rows.length > 0
								? `سطح بعدی پس از ذخیره: ${getLevelLabel(rows[rows.length - 1] as LevelRow, rows.length - 1)}`
								: "سطحی تعریف نشده است."}
						</div>
						<Button
							type="button"
