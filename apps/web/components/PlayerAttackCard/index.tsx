...
export default function PlayerAttackCard() {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			await proxyClientVoteAction(playerCode, body); // your function

			toast.success("عملیات ثبت شد", {
				description: "رأی شما با موفقیت ارسال شد",
			});

			setSelectedCode(null); // ← optional: clear after success
		} catch (err) {
	...
	const attacks = Object.entries(gameState.available_actions.attack || {});
	const defenses = Object.entries(gameState.available_actions.defense || {});
		...
		>
			<Card className="max-w-7xl w-full md:w-[1200px] bg-gray-900 border border-gray-700 shadow-xl rounded-xl overflow-hidden h-[calc(100vh-166px)] flex flex-col">
							...
							<section>
								...
								<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
									{attacks.map(([name, config]) => {
										const code = Object.entries(gameState.action_codes).find(
                        ...
                        `}
											>
												...
												<div className="text-sm text-gray-400">
													احتمال: {config.probability}% • هزینه: {config.cost}
												</div>
											</button>
										);
									})}
				...
				<CardFooter className="border-t border-gray-700 bg-gray-800/60 px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0">
					<div className="text-sm text-gray-400">
						نوبت {gameState.current_turn} از {gameState.total_turns} •{" "}
						{Math.floor(gameState.remaining_time / 60)}:
						{(gameState.remaining_time % 60).toString().padStart(2, "0")}{" "}
						باقی‌مانده
					</div>
					...
					<div className="flex gap-3">
						...
						<Button
							...
							disabled={isSubmitting || !selectedCode || !playerCode}
